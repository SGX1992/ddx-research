...
import { STUDY, PRIVACY_URL, DURATION_ESTIMATE, CONTACT_URL, BENEFITS, BRAND, THANKS, REEL_CAPTION } from './config.js?v=20260910120421';
import { createStore } from './state.js?v=20260910120421';
import { createSubmitter, probe } from './submit.js?v=20260910120421';
import { REEL, MEDIA_DIR } from './quotes.js?v=20260910120421';
import { renderSection, renderRail, el, bringIntoView, animate, stagger, syncOptions, syncMatrix } from './ui.js?v=20260910120421';
import { revealWords, reduced, EASE } from './motion.js?v=20260910120421';
...
/* One entry at a time, lazily. The loops are about a megabyte each, and this
   panel sits at the very end of the questionnaire — loading six of them up
   front would be a megabyte-a-piece tax on people who never reach it. */
function createReel(entries) {
  let i = 0;
  let media = null;
  let dots = null;
  let timer = null;
  ...
  function frame(entry) {
    const fig = el('figure', { class: 'showcase__quote' },
      el('blockquote', { text: `“${entry.quote}”` }),
      el('cite', { text: `${entry.name} — ${entry.role}` }));

    if (!entry.media) return fig;

    const img = loaded.get(entry.id) || el('img', {
      src: `${MEDIA_DIR}${entry.id}.gif`,
      alt: `${entry.name} answering the same question on video`,
      loading: 'lazy', decoding: 'async',
    });
    loaded.set(entry.id, img);
    /* If the loop is missing or blocked, the panel silently becomes the
       typographic card instead of showing a broken image. */
    img.onerror = () => { img.replaceWith(frame({ ...entry, media: false })); };
    return img;
  }
  ...
  function paint() {
    ...
    media.classList.toggle('has-media', Boolean(entry.media));
    media.replaceChildren(node);
    animate(node, [{ opacity: 0 }, { opacity: 1 }], { duration: 460 });

    /* The caption carries the attribution when the media is a loop. */
    const cap = media.parentElement?.parentElement?.querySelector('.showcase__cap');
    if (cap && entry.media) {
      cap.textContent = `${entry.name} — ${entry.role}. Speakers record a short version of this answer for DDX.`;
    }
    ...
    if (next.media && !loaded.has(next.id)) {
      const pre = new Image();
      pre.src = `${MEDIA_DIR}${next.id}.gif`;
      loaded.set(next.id, Object.assign(el('img', {
        alt: `${next.name} answering the same question on video`, decoding: 'async',
      }), { src: pre.src }));
    }
  }
  ...
  function run() {
    clearInterval(timer);
    /* Auto-advance is a nicety, not a mechanism — a respondent reading a
       quote should not have it pulled away, so reduced motion stops it. */
    if (!reduced) timer = setInterval(() => { i = (i + 1) % entries.length; paint(); }, 7000);
  }
  ...
  return {
    ...
    mount(mediaEl, dotsEl) {
      media = mediaEl;
      dots = dotsEl;
      dots.replaceChildren(...entries.map(() => el('i')));
      paint();
      run();

      /* An animated GIF keeps decoding whether or not anyone can see it.
         Scrolled out of view, or the tab in the background, it should cost
         nothing — this panel is a flourish and must never be why a phone gets
         warm halfway through a questionnaire. */
      io?.disconnect();
      if ('IntersectionObserver' in window) {
        io = new IntersectionObserver(([e]) => {
          if (e.isIntersecting) { if (!timer) run(); } else halt();
        }, { threshold: 0.15 });
        io.observe(mediaEl);
      }
      if (onVisibility) document.removeEventListener('visibilitychange', onVisibility);
      onVisibility = () => { if (document.hidden) halt(); else run(); };
